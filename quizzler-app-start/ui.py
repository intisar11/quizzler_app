...
class UserInterface:
    ...
    def get_next_question(self):
        ...
        if self.quiz.still_has_questions():
            quiz_t = self.quiz.next_question()
            self.canvas.itemconfig(self.question, text=quiz_t)
            self.score_label.config(text=f"Score: {self.quiz.score}")
        else:
            self.canvas.itemconfig(self.question, text="You have ended your quiz")
